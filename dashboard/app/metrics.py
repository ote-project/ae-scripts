...
def _best_value_column(columns: Iterable[str]) -> Optional[str]:
    for c in ValueCols:
        if c in columns:
            return c
    return None
...
def render_metrics_tab(run_dir: Path) -> None:
    ...
    - Lists files that contain `t` and at least one of rate, duration, or value columns
      (rates: m1_rate, mean_rate, m5_rate, m15_rate, count; durations: max, mean, min, stddev, p50, p75, p95, p98, p99, p999; values: value)
    ...
    for fp in files:
        ...
        if 't' in cols and (
            _best_rate_column(cols) or _best_duration_column(cols) or _best_value_column(cols)
        ):
            valid.append((_friendly_name(fp), fp, cols))

    if not valid:
        ui.label('No compatible metrics CSVs found (need t + rate/duration/value columns).').classes('text-negative')
        return
    ...
    preferred_series_order = ['m1_rate', 'mean', 'p50', 'count', 'value']
    default_y = (
        next((c for c in preferred_series_order if c in default_cols), None)
        or _best_rate_column(default_cols)
        or _best_duration_column(default_cols)
        or _best_value_column(default_cols)
        or 'value'
    )
    ...
    with ui.row().classes('items-end gap-4 flex-wrap'):
        ...
        sel_file = ui.select(file_options, value=str(default_fp), label='Metric', with_input=True).props('dense')

        def series_options(cols: List[str]) -> Dict[str, str]:
            ordered: List[str] = []
            for c in ['m1_rate', 'mean', 'p50', 'count', 'value']:
                if c in cols and c not in ordered:
                    ordered.append(c)
            for c in (RateCols + DurCols + ValueCols):
                if c in cols and c not in ordered:
                    ordered.append(c)
    ...
    def on_file_change(e=None):
        try:
            ...
            new_y = (
                next((c for c in preferred_series_order if c in cols), None)
                or _best_rate_column(cols)
                or _best_duration_column(cols)
                or _best_value_column(cols)
                or default_y
            )
